function CreateAccount(){
  ...
  function validate(field, label){
      if ((label === 'name' || label === 'email') && !field) {
        ...
        setStatus('Erros: ' + label + ' must be greater than eight characters.');
        return false;
      }
      return true;
  }
  ...
  return (
    <Card
      ...
      body={show ? (  
              ...
              <button type="submit" className="btn btn-light" onClick={handleCreate} disabled={submitDisbaled}>Create Account</button>
              </>
            ):(
              <>
              <h5>Success</h5>
